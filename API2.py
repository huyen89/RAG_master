...
import os
file_paths = ['data/Go_Japan.pdf', 'data/data.pdf']

for file_path in file_paths:
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"File {file_path} not found.")
    
os.environ["LLAMA_CLOUD_API_KEY"] = 'llx-q0S3D5OSSDpEaDzoRBYbscBzoE3d3JSWFO1MfATJQV5wlYc0'
nest_asyncio.apply()
...
CASE DATA: 'HOẠT ĐỘNG NGOẠI KHÓA'
    EXAMPLE 1:
        ...
        Step 1: Indentify key elements of the text, as well as previous chat history if available:
...
def run_openai_task(labels, text, data=None):
    if data is None:
        data = []
